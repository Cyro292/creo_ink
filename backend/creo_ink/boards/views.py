from rest_framework.generics import ListAPIView, CreateAPIView, UpdateAPIView, RetrieveUpdateDestroyAPIView, ListCreateAPIView, RetrieveAPIView
...
class JoinBoardByLinkView(RetrieveAPIView):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if board_id is None:
            return Response({"detail": "Invalid or expired link token."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"message": "Successfully joined the board."}, status=status.HTTP_200_OK)
