    ...
    if request.method == "POST":
        if "permission" in request.POST:
            ...
            if change_user_permission_form.is_valid():
                ...
                if not board.has_access(
                    user=request.user, min_permission=models.Participation.ADMIN
                ):
                    messages.info(
                        request,
                        "you have to be at least admin in order to change permissions",
                    )

                if not target_permission >= user_permission:
                    messages.info(
                        request,
                        "you are not allowed to set higher positions then yourself",
                    )
...
from .serializers import UserSerializer, BoardSerializer, UserBoardSerializer, BoardPermissionSerializer
...
class BoardUsersListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Participation.objects.filter(board=board)
...
class BoardAdministerUsersView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            participation = Participation.objects.get(board=board, user=user)
            participation.permission = permission
            participation.save()
        except Participation.DoesNotExist:
            Participation.objects.create(board=board, user=user, permission=permission)

        return Response(status=status.HTTP_200_OK)

class BoardKickUserView(generics.DestroyAPIView):
    serializer_class = BoardPermissionSerializer
    permission_classes = (IsAuthenticated,)
...
# Add the remaining views for other API endpoints
